...
class TwitterThread(QThread):
    ...
    def run(self):
        ...
        keyword,Start,Stop,dir,status = self.parent().x
        ...
        if self.keyword == "" :
                ...
                # มีแล้วให้ข้าม Search
                    ...
                    if self.status == "notupdate":
                        if self.keyword_file not in self.dir_inkey:
                            ...
                            self.lst_keyword = []
                            self.lst_keyword.append(self.keyword)
                            self.status_backup = "update_backup"
                            self.ma_tw.backup_data(self.dir,self.lst_keyword,self.status_backup)
                        ...
                        print("ManualUpdate..."+r"Date : {} / Keyword : {}".format(self.Start_dt.date(),self.keyword))
                        self.ma_tw.main(self.keyword,str(self.Start_dt.date()))
                        ...
                        self.lst_keyword = []
                        self.lst_keyword.append(self.keyword)
                        self.ma_tw.backup_data(self.dir,self.lst_keyword,self.status)
            ...
            self.merge_dataframe = self.ma_tw.merge_df(self.list_df)   
            self.back_dataframe  = self.ma_tw.back_df_notsave(self.list_df)
            
            self.re              = self.ma_tw.relatedword(self.ma_tw.merge_df(self.list_df))
    ...
    def get (self):
        ...
        merge_dataframe = self.merge_dataframe
        back_dataframe  = self.back_dataframe
        related = self.re
        return [merge_dataframe,back_dataframe,related]
