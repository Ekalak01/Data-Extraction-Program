...
class pop_main_wb(QtWidgets.QDialog):
    ...
    def export(self):
        try:
            ...
            start=set_startdatestr
            stop=set_stopdatestr
            dir=parent_dir+'/Appfolder/export/'+keyword+'/'
            if not os.path.exists(dir):
                os.mkdir(dir)
            savepath=dir+'/'+keyword+' '+start+' to '+stop+'.csv'
            df=self.web_worker.get()[0]

            self.dfreflinks.to_csv(dir+'/'+keyword+' ref '+start+' to '+stop+'.csv',index=False)
            self.dfst.to_csv(dir+'/'+keyword+' statics '+start+' to '+stop+'.csv',index=False)
            self.dfword.to_csv(dir+'/'+keyword+' related word '+start+' to '+stop+'.csv',index=False)
    ...
    def keepreflinks(self,df_all):
        try:
            self.dfreflinks=self.dfmanage.reflinks(df_all)
            self.model = PandasModel(self.dfreflinks)
            self.tableView_ref.setModel(self.model)
            self.tableView_ref.setSortingEnabled(True)
            self.tableView_ref.sortByColumn(1, Qt.DescendingOrder) 
            return  self.dfreflinks
        except:
            pass
    
    def keeprelatedword(self,df_all:pd.DataFrame):
        try:
            self.dfword=self.dfmanage.relatedword(df_all)
            self.model = PandasModel(self.dfword)
            self.tableView_word.setModel(self.model)
            self.tableView_word.setSortingEnabled(True)
            self.tableView_word.sortByColumn(1, Qt.DescendingOrder) 
            return self.dfword
        except:
            pass
