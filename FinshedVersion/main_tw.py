...
class MySignal(QObject):
    ...
    object_signal_x = pyqtSignal(colck_x)
    
class MyThreadxy(QThread):
    ...
    def __init__(self, val):
        ...
        self.object_signal_x = MySignal().object_signal_x
    ...
    def run(self):
        ...
        while(1):
            ...
            #print(time_val)
            self.object_signal_x.emit(colck_x(time_val))
...
class pop_main_tw(QtWidgets.QDialog):
    ...
    def __init__(self,parent=None):
        ...
        self.object_signal_x = self.my_signal.object_signal_x
        self.object_signal_x.connect(self.show_it)
    ...
    def export(self):
        ...
        start= self.set_startdatestr
        self.Start =  datetime.datetime.strptime(start, "%Y-%m-%d")
        stop=self.set_stopdatestr
        self.Stop = datetime.datetime.strptime(stop, "%Y-%m-%d")
        parent_dir = QtCore.QDir.currentPath()
        pathx = parent_dir+'/Appfolder'+"/"+'export/'+self.tw
        if not os.path.exists(pathx):
            os.mkdir(pathx)
            
        savepath= pathx+'/'+self.tw+'[ '+str(self.Start.date())+' to '+str(self.Stop.date())+' ]'+'.csv'
        
        k = self.twitter_worker.get()
        
        df = k[0]
        df.to_csv(savepath,index=False)
        
        savepath_word= pathx+'/'+self.tw+'= Relate word ='+'[ '+str(self.Start.date())+' to '+str(self.Stop.date())+' ]'+'.csv'
        df_word = self.dfrelate
        df_word.to_csv(savepath_word,index=False)
        
        
        savepath_statics= pathx+'/'+self.tw+'= Statics ='+'[ '+str(self.Start.date())+' to '+str(self.Stop.date())+' ]'+'.csv'
        df_statics = self.keep_statics_x
        df_statics.to_csv(savepath_statics,index=False)
        
        savepath_Hastag= pathx+'/'+self.tw+'= Relate Hastag ='+'[ '+str(self.Start.date())+' to '+str(self.Stop.date())+' ]'+'.csv'
        df_Hastag = self.keep_re_df_x 
        df_Hastag.to_csv(savepath_Hastag,index=False)
        
        
        Update_Popup = Dialog_Complete()
        Update_Popup.exec()
    ...
    def op_pd(self):
        print("Opend Pandas Loading Put in...")
        try :
            k = self.twitter_worker.get()
            self.addtotable(k[0])
            self.keep_statics(k[1])
            self.keeprelatedword(k[2])
        except :
            pass
    ...
    def keep_statics(self,keep_st_df):
        try:
            ...
            pass
        try:
            self.keep_statics_x = keep_st_df
    ...
    def keeprelatedword(self,df_all:pd.DataFrame):
        print(df_all)
        self.dfrelate = df_all
        self.model = PandasModel(df_all)
        self.tableView_word.setModel(self.model)
        self.tableView_word.setSortingEnabled(True)
        self.tableView_word.sortByColumn(1, Qt.AscendingOrder) 
