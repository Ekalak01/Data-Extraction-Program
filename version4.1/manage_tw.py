...
class manage_tw:
    ...
    def search_twitter(self,keyword,Start):
        ...
        # Create data frame
        df = pd.DataFrame(columns=['Index','Keyword','Date', 'Text', 'Hashtag', 'Sentiment', 'Retweet' , 'NLP'])
        ...
        for tweet in tweets:
			...
			# Add in table	
            new_columns = pd.Series([index_i,query,date,text_intercept,hashtag,text_sentiment,retweet_cnt,text_tokenize], index=df.columns)
